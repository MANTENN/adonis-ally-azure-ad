...
import {
  ...
  AzureActiveDirectoryDriverConfig,
  AzureActiveDirectoryScopes,
  UserFields,
  UserFieldsAndToken,
  UserInfo,
 ...
 * ------------------------------------------------
 */
export class AzureActiveDirectory extends Oauth2Driver<
  AzureActiveDirectoryToken,
  AzureActiveDirectoryScopes
> {
  /**
  ...
  protected authorizeUrl = `https://login.microsoftonline.com/${
    this.config.tenantId ? this.config.tenantId : 'common'
  ...
  constructor(ctx: HttpContextContract, public config: AzureActiveDirectoryDriverConfig) {
    super(ctx, config)
   ...
   */
  protected configureRedirectRequest(request: RedirectRequest<AzureActiveDirectoryScopes>): void {
    /**
  ...
  ): Promise<AllyUserContract<AzureActiveDirectoryToken>> {
